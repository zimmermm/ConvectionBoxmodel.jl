...
function solve_boxmodel(lakemodel; saveat=[])
	...
	# solve
	if isempty(saveat)
		@time solve(prob, Rosenbrock23(autodiff=false), reltol=1.0e-2, abstol=1.0e-2, dtmin=1.0/24.0/60.0, dtmax=1.0/24.0/12.0)
	else
		@time solve(prob, Rosenbrock23(autodiff=false), reltol=1.0e-2, abstol=1.0e-2, dtmin=1.0/24.0/60.0, dtmax=1.0/24.0/12.0, saveat=saveat)
	end
end
